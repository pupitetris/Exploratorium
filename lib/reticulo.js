      ...
      function csvLoaded (datos) {
        ...
        function latticeJsonLoaded(graph) {
            ...
            if (d.labelAttributes.length > 0) {
              ...
              for (var i = 0; i < d.labelAttributes.length; i++) {
                ...
                var textBox = group.append(
                  createTextBox(eleClass, x + "," + y, attr)
                );
          ...
          function darInfo(event) {
            ...
            infobox.select("button.close").on("click", close);

            var coordInd = d3.select("#attr-desc-" + attr_id + " .attr-desc-ind").html();
            var exp = d3.select("#attr-desc-" + attr_id + " .attr-desc-exp").html();
