(function() {
  ...
  function reticulo() {
    ...
    function keyboxSetup(keybox, classDescriptions) {
      var cont = keybox.select(".cont");
      for (const desc of classDescriptions)
        cont.append("div")
          .html(`<span class="textbox-box box-${desc.key}"></span>${desc.desc}`);
      floatboxSetup(keybox)
        .style("display", "block");
    }

    function nodeDragSetup(nodes, links) {
      function dragstart(event, datum) {
        d3.select(this)
          .classed("drag", true);
      }
    ...
    function toolbarSetup(shell, svg, zoom) {
      ...
      shell.addEventListener("fullscreenchange", () => toolbarFullscreenChange(shell, fsbtn));
    }
    ...
    this.main = function() {
      ...
      toolbarSetup(d1.node().parentNode, svg, zoom);
      ...
      var nodeDrag = nodeDragSetup(nodes, links);
      ...
      function csvLoaded(datos) {
        ...
        function latticeJsonLoaded(graph) {
          ...
          keyboxSetup(d3.select("#keybox"), graph.classDescriptions);
