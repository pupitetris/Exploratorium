(function() {
  ...
  function reticulo() {
    ...
    function legendSetup(legend, toolbar, classDescriptions) {
      ...
      floatboxSetup(legend, () => toolbar.select(".tool-infobox").classed("active", false))
        .style("display", "block");
    }
    ...
    function toolbarSetup(shell, svg, zoom, legend) {
      ...
      tb.select(".tool-infobox")
        .on("click", (event) =>
          legend.style("display", (d3.select(event.target).classed("active"))? "block": "none"));

      tb.select(".tool-attrs")
        .on("click", (event) =>
          svg.classed("no-attributes", !d3.select(event.target).classed("active")));

      tb.select(".tool-levels")
        .on("click", (event) =>
          svg.classed("show-levels", d3.select(event.target).classed("active")));
