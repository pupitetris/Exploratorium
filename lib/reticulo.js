(function() {
  ...
  function reticulo() {
    ...
    function createTextBox(eleClass, offsetX, offsetY, text) {
    ...
    function editorSetup(editor, activate, diagram, graph, viewBox) {
      ...
      controls.btnViewboxSet
        .on("click", function() {
          ...
          diagram.select("svg").attr("viewBox",
    ...
    function legendSetup(legend, toolbar, classDescriptions) {
      ...
      for (const desc of classDescriptions)
        cont.append("div")
        .html(`<span class="figure textbox-box box-${desc.key}"></span>${desc.desc}`);
    ...
    this.main = function() {
      ...
      function latticeJsonLoaded(graph) {
        editorSetup(d3.select("#editor"), that.config.USE_EDITOR, d1, graph, viewBox);
